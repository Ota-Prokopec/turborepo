...
{#if !disabled}
	<Button
		{id}
		class={twMerge(
			'  flex justify-center items-center text-white bg-blue-500 p-4 pl-8 pr-8 text-xl',
			className
		)}
		on:click
	>
...
{:else}
	<Button
		...
		disabled
		class={twMerge(
			' flex justify-center items-center bg-slate-700 text-white p-4 pl-8 pr-8 text-xl',
			className,
			classForDisabled
		)}
	>
