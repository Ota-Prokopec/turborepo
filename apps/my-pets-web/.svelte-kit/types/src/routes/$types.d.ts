...
type LayoutRouteId = RouteId | "/(app)/(pets)" | "/(app)/(pets)/addpet" | "/(app)/(pets)/notfound" | "/(app)/(pets)/pet/[petId]" | "/(app)/(pets)/pet/[petId]/edit" | "/(app)/account/settings" | "/auth" | "/auth/oauth2/failure" | "/auth/oauth2/success" | null
type LayoutParams = RouteParams & { petId?: string }
